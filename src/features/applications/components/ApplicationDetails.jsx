...
import { clearSelectedApplication } from "../store/applicationsSlice";
...
const ApplicationDetails = () => {
  ...
  const app = useSelector((state) =>
    state.applications.applications.find((a) => String(a.id) === String(id))
  );
  ...
  const dashArray     = `${(app.matchScore / 100) * circumference}, ${circumference}`;
        ...
        <div className="flex-1 lg:w-3/5 flex flex-col gap-6">
          ...
          <div className="relative rounded-2xl bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/50 backdrop-blur-sm p-6 shadow-sm overflow-hidden">
            ...
            <div className="flex items-start gap-4 mb-2">
              <div className={`h-20 w-20 rounded-2xl flex items-center justify-center text-3xl font-bold shadow-inner border shrink-0 bg-gradient-to-br ${app.gradientFrom} ${app.gradientTo} text-white`}>
                {app.initial}
              </div>
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2 text-sm flex-wrap">
                  <span className="font-semibold text-slate-700 dark:text-slate-200 text-lg">{app.company}</span>
              ...
              <div className="flex flex-col items-center shrink-0">
                <div className="relative h-12 w-12 flex items-center justify-center">
                  <svg className="h-full w-full -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path
                      ...
                      strokeDasharray={dashArray} strokeWidth="3"
                    />
                  </svg>
                ...
                <span className={`text-xs font-semibold mt-1 ${app.matchColor}`}>
                  {app.matchLabel}
                </span>
              </div>
            </div>
          ...
          <div className="flex-1 flex flex-col bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/50 backdrop-blur-sm rounded-2xl shadow-sm overflow-hidden">
            <div className="flex items-center border-b border-slate-200 dark:border-slate-700/50 px-2 pt-2 overflow-x-auto">
              {TABS.map((tab) => (
                <button
                  key={tab}
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {activeTab === "Overview" ? (
                <div className="prose prose-sm dark:prose-invert max-w-none text-slate-600 dark:text-slate-300">
                  <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">About the Role</h3>
                  ...
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-2">Benefits:</h4>
        ...
        <div className="w-full lg:w-[30%] space-y-6">
            ...
            <div className="space-y-5">
              {app.statusHistory.map((s, i) => (
                <div key={i} className="flex gap-3 relative">
                  <div className={`h-6 w-6 rounded-full ${s.color} flex items-center justify-center shrink-0 z-10`}>
          ...
          <div className="rounded-2xl bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/50 backdrop-blur-sm p-6 shadow-sm">
            ...
            <div className="space-y-4">
              {[
                { icon: "description", iconBg: "bg-red-50 text-red-500",  name: "Resume_Custom.pdf",       sub: `Optimized for ${app.company}` },
                { icon: "article",     iconBg: "bg-blue-50 text-blue-500", name: "Cover_Letter_Final.pdf", sub: "Generated by AI" },
              ].map((m) => (
                <div key={m.name} className="p-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-blue-400 transition-colors bg-slate-50 dark:bg-slate-900/30">
                  <div className="flex items-center gap-3">
                    <div className={`h-10 w-10 rounded ${m.iconBg} flex items-center justify-center shrink-0`}>
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    <button className="flex-1 text-[10px] font-bold py-1.5 rounded bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 transition-colors flex items-center justify-center gap-1 uppercase">
                      <span className="material-symbols-outlined text-sm">autorenew</span> Regenerate
                    </button>
                  </div>
                </div>
              ))}
          ...
          <div className="rounded-xl p-6 bg-slate-800 dark:bg-slate-900 text-white shadow-lg overflow-hidden relative">
            <div className="relative z-10">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">JobTrackr Insight</h3>
              <p className="text-sm font-medium leading-relaxed">
                Your profile matches{" "}
                <span className="text-blue-400 font-bold">{app.matchScore}%</span>{" "}
            ...
            <span className="material-symbols-outlined absolute -right-4 -bottom-4 text-7xl text-white/5">psychology</span>
          </div>
        </div>
      </div>
    </div>
